...
class Visualizer:
    def __init__(self
        ...
        if app_data.empty:
            ...
            self.plot_palette = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf']

    def histogram_layout(self, column_name, start_value, end_value, bins, xaxis_title):
        histogram = go.Figure()
        histogram.add_trace(go.Histogram(
            x=self.app_data[column_name],
            nbinsx=bins,
            marker=dict(color=self.plot_palette),
            xbins=dict(start=start_value, end=end_value, size=(end_value - start_value) / bins)
        ))
        histogram.update_xaxes(title_text=xaxis_title)
        histogram.update_yaxes(title_text='Frequency')
        histogram.update_layout(margin=dict(l=0, r=0, t=0, b=0), bargap=0.01)
        return histogram
    ...
    def plot_bubble_chart(self):
        bubble_trend = go.Figure()
        bubble_trend.add_trace(go.Scattergl(
            x='Carga Tributária Municipal 2010',
            y='IDHM 2010',
            mode='markers',
            marker=dict(size='Habitantes 2010'),
            text='Município',
            trendline='lowess',
            width=550,
            height=275,
        ...
        bubble_trend.update_xaxes(title_text='Carga Tributária Municipal 2010')
        bubble_trend.update_yaxes(title_text='IDHM 2010')
        bubble_trend.update_layout(margin=dict(l=0, r=0, t=0, b=0))
        return bubble_trend
    ...
    def plot_correlation_heatmap(self):
        ...
        corr_heatmap.add_trace(go.Heatmap(
